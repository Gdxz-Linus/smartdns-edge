...
此项目源自mokeyish的项目0.13版为基础的二次开发，借助免费人工智能引擎，实现双栈优选等功能。绝无dns泄漏，也无内存泄漏。
...
SmartDNS-edge同时支持指定特定域名IP地址，并高性匹配，可实现过滤广告的效果，多上游解析分流等功能。

目前已经过了windows、wsl2（linus）平台重点功能的测试。
...
1.	连接数有上限：同时连上来的总数、以及单个来源的连线数都有上限；超了只拒绝新连接，不会把正在用的连接踢掉。上限按机器的物理内存自动推算（小机器自动收紧、大机器自动放宽），也可以按端口单独调整。
2.	不支持的请求会得到明确答复：遇到老式或不认识的查询类型，程序会回一个“不支持”的应答，而不是像过去那样一声不吭地丢弃。
3.	伪造的“应答包”直接丢弃：这类报文多来自伪造或反射流量，对它再作答会形成回环，因此只记录、不回应。
4.	只采纳真正来自所查上游的应答：来源对不上的应答一律丢弃，防止解析结果被第三方顶替。
5.	出问题看得见、有兜底：程序内部万一出错，会把情况记进日志并计数；该请求会尽量回一个“服务器故障”的应答，不让客户端一直等到超时。
