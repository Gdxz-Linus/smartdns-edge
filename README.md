SmartDNS-edge是C语言Smartdns的rust版本，基本功能与C语言版本一致，完美实现各项功能，并对缓存预取、双栈优选、测速等算法逻辑进行了充分优化，全方位超越C语言版，完全可作为C语言版本的平替；

此项目源自mokeyish的项目0.13版为基础的二次开发，借助免费gemini 3.1 pro智能引擎，实现多上游解析分流，双栈优选等功能，无dns泄漏，业务内存泄漏。

本次重构保持原有架构不变，对业务逻辑、网络吞吐、CPU性能、内存防泄漏、反网络攻击和容灾防御上进行了彻底的重塑，全面对标工业级、企业级的边缘DNS代理网关。

SmartDNS-edge接受来自本地或局域网客户端的DNS查询请求，然后从多个上游DNS服务器获取DNS查询结果，并将访问速度最快的解析结果返回给客户端，以此提高网络访问速度。

SmartDNS-edge同时支持指定特定域名IP地址，并高性匹配，可达到过滤广告的效果，实现多上游解析分流，绝无dns泄漏，也无内存泄漏。

由于条件有限，目前经过了windows平台重点功能的测试，未进行Linux、Macos、Docker平台的测试。

## 改进的功能
🚀 SmartDNS-edge 核心架构与功能演进

💡 第一大块：核心功能与业务逻辑完整性
本版块深度修补了旧版在协议标准 (RFC)、跨端兼容性、规则解析上的盲区，在极端场景下做到了“企业级可用”。

📦一、 缓存数据库全面升级（契合 RFC 规范与自愈能力）
1.	双栈并行防空包机制：双栈（A/AAAA）并发查询时，若一方返回无IP的空包，系统会等待另一方的结果。避免因单栈空包抢答导致解析失败，提升了单栈或污染网络环境下的解析成功率。
2.	【重磅】缓存数据库完整保留三大区（重磅）：重构了底层的序列化结构，现在能完整保存DNS报文的Answer（应答）、Authority（权威）和Additional（附加）三大区。使缓存完全符合EDNS0及RFC规范，解决客户端因缓存信息不全导致的解析异常。
...
7.	【重磅】增加否定缓存机制：无论是单站优选还是双栈优选，空记录也会存入缓存。避免无否定缓存造成的缓存击穿，解决因此引发的上游无限查询重试风暴。
8.	【重磅】缓存预取超时与热度限制：新增serve_expired_prefetch_time预取超时参数（默认6小时）。缓存达到预取超时时间并且访问热度大于等于2才会触发预取，否则进入过期超时队列。过期缓存被后台预取后，热度强制重置为1，需再次被真实访问才触发下次预取；杜绝了冷门域名无限预取导致僵尸缓存和消耗后台流量的问题。
...
💾三、  硬盘 I/O 极限降载
1.	【重磅】后台任务与核心解析彻底隔离（防假死）：将读取海量配置文件、热重载去广告规则等所有耗时操作，全部剥离到后台专属的阻塞线程池去处理，并将该池子容量扩充至2048 个线程。解决更新规则或执行复杂任务时，导致主线程假死的问题。
...
📡三、  资源上限与协议健壮性（防拖垮、防崩溃）
1.	不信任报文长度前缀：TCP/DoT/DoQ 的报文带 2 字节长度声明（最大 65535）。旧做法一收到长度就按声明分配内存，攻击者发 2 个字节就能白占 64 KiB；现在改为按 4 KiB 分块、按需增长，内存只与实际收到的字节数成正比。
2.	首包超时：连接建立后 5 秒内必须发来第一个完整报文（`first-packet-timeout`），否则断开；已建立连接的长连接复用不受影响，仍按 `tcp-idle-time` 空闲超时。
3.	连接数上限：同时连接总数与单一来源连接数都有上限，超限只拒绝新连接、不断开已有连接。默认按物理内存自动推算（内存预算 = clamp(物理内存/8, 16 MiB, 512 MiB)，总上限 = clamp(预算/32 KiB, 512, 16384)，单来源上限 = 总上限/8），本机环回不占配额；可用 `max-connections` / `max-connections-per-ip` 调整，也可在某条监听上单独收紧，如 `bind-tcp 0.0.0.0:53 -max-connections 20000`。出口经 NAT 的网络请调大单来源上限。
4.	状态可观测：当前连接数、被拒次数、总上限、以及崩溃次数（`panics_total`，正常恒为 0）都可通过 `GET /api/system/status` 查看，便于按真实峰值决定是否调整上限。
5.	不支持的请求类型明确应答：收到 IQUERY / STATUS / NOTIFY / UPDATE 或其它未知 OpCode 时，按 RFC 1035 §4.1.1 回 NotImp（并回带原始 ID 与 Question 段），不再静默丢弃。
6.	收到“响应包”（QR=1）静默丢弃：这类报文通常来自伪造或反射流量，对响应再作响应会形成回环，因此只记录、不回应。
7.	上游应答来源校验：直连的 UDP 上游套接字会 connect 到该上游，来源 IP/端口不符的应答由内核丢弃；经 SOCKS5 代理时另做数据报头来源校验，不符则丢弃并计数。
8.	崩溃可观测与兜底应答：进程内任何 panic 都会写进日志并计数；万一仍有未预期的 panic，该请求会尽量回一个 SERVFAIL，而不是让客户端一直等到超时。
...
- **云服务器 / 边缘计算节点 (VPS & Edge)**：
  ...
  - **【重点】**完美支持 AWS Graviton、阿里云倚天、Oracle等高性能 ARM64 云服务器。
