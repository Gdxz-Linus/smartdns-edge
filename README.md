...
1.	拒绝“小马拉大车”式的内存占用：客户端声称的报文长度不再直接当作内存申请的依据，实际收到多少才占多少。有人只发两个字节就想占住 64 KiB 的老套路，在这里行不通。
2.	连接不许干等：连接建立后 5 秒内必须把第一个完整请求发上来，否则断开；已经在用的长连接不受影响。
3.	连接数有上限：同时连上来的总数、以及单个来源的连线数都有上限；超了只拒绝新连接，不会把正在用的连接踢掉。上限按机器的物理内存自动推算（小机器自动收紧、大机器自动放宽），也可以按端口单独调整。本机（127.0.0.1）永远不占配额，这样即使被人用连接洪水打满，你仍能进管理后台处理。
4.	情况看得见：当前连接数、被拒绝的次数、上限值，以及程序自身的异常次数，都能在管理后台里查看。建议先按默认值运行一段时间，看清真实峰值再决定要不要调整。
5.	不支持的请求会得到明确答复：遇到老式或不认识的查询类型，程序会回一个“不支持”的应答，而不是像过去那样一声不吭地丢弃。
6.	伪造的“应答包”直接丢弃：这类报文多来自伪造或反射流量，对它再作答会形成回环，因此只记录、不回应。
7.	只采纳真正来自所查上游的应答：来源对不上的应答一律丢弃，防止解析结果被第三方顶替。
8.	出问题看得见、有兜底：程序内部万一出错，会把情况记进日志并计数；该请求会尽量回一个“服务器故障”的应答，不让客户端一直等到超时。
