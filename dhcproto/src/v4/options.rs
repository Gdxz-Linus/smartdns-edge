...
use hickory_proto::{
    rr::Name,
    serialize::binary::{BinDecodable, BinDecoder, BinEncodable, BinEncoder, EncodeMode, NameEncoding},
};
use ipnet::Ipv4Net;
#[cfg(feature = "serde")]
        ...
        match self {
            ...
            O::ClientFQDN(fqdn) => {
                ...
                if flags.e() {
                    ...
                    //
                    // 本地适配（见 dhcproto/VENDORED.md）：内嵌的 hickory-proto 0.26
                    // 把 Name::emit_as_canonical(&mut enc, true) 换成了"由编码器决定
                    // 大小写与压缩"，即 set_name_encoding(NameEncoding::UncompressedLowercase)
                    // + 标准的 BinEncodable::emit。语义与原来的 canonical=true 等价：
                    // 标签转小写、不做压缩。
