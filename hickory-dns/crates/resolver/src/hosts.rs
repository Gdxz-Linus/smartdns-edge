...
impl Hosts {
    ...
    pub fn read_hosts_conf(&mut self, mut src: impl io::Read) -> io::Result<()> {
        // 🌟 优化 1：一次性将整个文件内容读入单个大 String 中，极大降低底层系统 I/O 开销
        ...
        // 🌟 优化 2：使用 content.lines()。它返回的是 &str 切片，指向大 String 的局部。
        // 这彻底消灭了原本在循环中，为“每一行”都单独在堆上分配一个 String 的愚蠢开销！
        for (line_index, mut line) in content.lines().enumerate() {
            // Remove byte-order mark if present
            if line_index == 0 && line.starts_with('\u{feff}') {
                // BOM is 3 bytes
                line = &line[3..];
            }
            ...
            for domain in iter {
                // 🌟 优化 3：由于 99.9% 的 hosts 规则都是纯小写，我们只在检测到大写字母时才执行 to_lowercase()。
                // 这样避免了为“每一个域名”都高频深拷贝 String 的内存黑洞，对大型 hosts 文件实现了零拷贝解析！
                let mut name = if domain.chars().any(|c| c.is_uppercase()) {
                    let domain_lower = domain.to_lowercase();
                    match Name::from_str(&domain_lower) {
                        Ok(name) => name,
                        Err(_) => continue,
                    }
                } else {
