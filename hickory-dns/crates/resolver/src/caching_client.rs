    ...
    C: DnsHandle + Send + 'static,
{
    ...
    // 🌟 核心优化：直接将 Lookup 插入缓存，无需再构造多余的临时 Message 对象
    fn cname(&self, lookup: Lookup, query: Query) -> Result<Lookup, ProtoError> {
        self.cache.insert(query, Ok(lookup.clone()), Instant::now());
        Ok(lookup)
    }

    // 🌟 核心优化：直接将 Lookup 插入缓存，完全剪除了旧版的 answers/Message 打包动作
    fn cache(
        &self,
        query: Query,
        ...
        self.cache.insert(query, Ok(lookup.clone()), now);
