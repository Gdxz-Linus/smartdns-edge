...
use futures_util::future::Either; // 🌟 新增导入
    ...
    C: DnsHandle + 'static,
{
    ...
    options: DnsRequestOptions,
    query: Either<
        future::Ready<Result<Lookup, ProtoError>>,
        Pin<Box<dyn Future<Output = Result<Lookup, ProtoError>> + Send>>,
    >, // 🌟 优化：改用无堆分配的 Either 双路选择器
}
        ...
        let query = match name {
            Ok(name) => {
                ...
                if let Some(lookup) = hosts.lookup_static_host(&query) {
                    Either::Left(future::ok(lookup)) // 🌟 优化：hosts 命中直接走栈，0 堆分配！
                } else {
                    Either::Right(client_cache.lookup(query, options)) // 🌟 优化：直接使用 caching_client 已有的 Box，不进行二次装箱
                }
            }
            Err(err) => Either::Left(future::err(err)), // 🌟 优化：直接报错，0 堆分配！
        };
    ...
    C: DnsHandle + 'static,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        loop {
            // 🌟 修复：安全获取可变引用
            let mut_self = self.as_mut().get_mut();
            
            // Try polling the underlying DNS query.
            let query = mut_self.query.poll_unpin(cx);
            ...
            if should_retry {
                if let Some(name) = mut_self.names.pop() {
                    let record_type = mut_self.record_type;
                    let options = mut_self.options;

                    // 🌟 优化：重试时重新封装进右支
                    mut_self.query = Either::Right(mut_self.client_cache.lookup(Query::query(name, record_type), options));
                    continue;
                }
            }
