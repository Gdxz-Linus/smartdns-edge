...
impl<C: DnsHandle + 'static> LookupIpFuture<C> {
    ...
    pub fn lookup(
        ...
        Self {
            ...
            strategy,
            client_cache,
            // 🌟 优化：占位错误走 Left 栈路径，消除首次创建时的堆分配
            query: Either::Left(future::err("can not lookup IPs for no names".into())),
            options,
            hosts,
            finally_ip_addr,
        }
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        loop {
            ...
            if let Some(name) = mut_self.names.pop() {
                // 🌟 优化：外发策略查询时封装进右支
                mut_self.query = Either::Right(LookupContext {
                    client: mut_self.client_cache.clone(),
                    options: mut_self.options,
                    hosts: mut_self.hosts.clone(),
                }
                .strategic_lookup(name, mut_self.strategy)
                .boxed());
                continue;
            } else if let Some(ip_addr) = mut_self.finally_ip_addr.take() {
