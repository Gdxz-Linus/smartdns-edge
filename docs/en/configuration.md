...
| server-https | Upstream HTTPS DNS server | None | Repeatable. <br />https://[host][:port]/path: Server URL. <br />[-http-host]: the Host header of the DoH request (separate from TLS SNI, which stays the name in the address). When omitted it is derived from the address: the domain as-is, or `IP` / `IP:port` (port omitted for 443). <br /> Plus all `server-tls` options. | server-https https://cloudflare-dns.com/dns-query |
